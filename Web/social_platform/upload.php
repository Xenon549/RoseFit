...
if (file_exists($target_file)) {
    echo "上傳的檔案已存在，請重新上傳。";
    echo "<meta http-equiv=REFRESH CONTENT=5;url=\"http://120.110.113.70/rosefit_greenhouse/social_platform/profile_manage.php?id=".$rowname[0]."\">";
    $uploadOk = 0;
}
// Check file size
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $upload_filename)) {
        ...
        $upload = "http://120.110.113.70/rosefit_greenhouse/social_platform/profile/".$changename;
        $sql = "UPDATE user set user_sticker='$upload'where user_email='$email'";
        if(mysqli_query($link , $sql))
        {
                echo '修改大頭貼成功！畫面將自動跳轉，請稍候';
                echo "<meta http-equiv=REFRESH CONTENT=3;url=\"http://120.110.113.70/rosefit_greenhouse/social_platform/profile_manage.php?id=".$rowname[0]."\">";
        }
        else{
